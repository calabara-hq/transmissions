/**
 * Modified from zora-protocol/1155-deployments
 */
...
async function copyEnvironmentRunFiles() {
  const latestFiles = await glob(`broadcast/UpgradeChannels.s.sol/**/*run-latest.json`);
  ...
  const allFileContents = await Promise.all(
    latestFiles.map(async (file: string) => {
      ...
      const filtered = returns.replace(/\\/g, "").replace('"{', "{").replace('}"', "}");
  ...
  const withLatest = Object.entries(groupedByChainId).map(([chainId, files]: any) => {
    const latest = files.sort((a: any, b: any) => b.timestamp! - a.timestamp!)[0];
    return {
      chainId,
      latest,
    };
