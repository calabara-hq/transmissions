...
contract FiniteChannelTest is Test {
  ...
  function initializeChannelWithTimingScenario(uint80 createStart, uint80 mintStart, uint80 mintEnd) internal {
    ...
    targetChannel.initialize(
      ...
      new address[](0),
      new bytes[](0),
      createEncodedFiniteParams(createStart, mintStart, mintEnd, ranks, allocations, 1, NativeTokenLib.NATIVE_TOKEN)
    );
  }
  ...
  function createParamsFromFuzzedInputs(
    FuzzedInputs memory inputs
  ) internal returns (uint40[] memory _ranks, uint256[] memory _allocations, uint256 _totalAllocation) {
  ...
  function test_finiteChannel_sortWinners(FuzzedInputs memory inputs) public {
    (uint40[] memory _ranks, uint256[] memory _allocations, uint256 _totalAllocation) = createParamsFromFuzzedInputs(
      inputs
    );
