...
contract FeesTest is Test {
  ...
  function _mockBatchedMint(
    ...
    address sponsor,
    uint256 batchSize
  ) internal returns (Rewards.Split memory, Rewards.Split memory) {
  ...
  function test_customFees_allocations(uint256 batchSize) public {
    ...
    (Rewards.Split memory ethSplit, Rewards.Split memory erc20Split) = _mockBatchedMint(
      creator,
      referral,
      sponsor,
      batchSize
  ...
  function test_customFees_nullChannelTreasury() public {
    ...
    (Rewards.Split memory ethSplit, Rewards.Split memory erc20Split) = _mockBatchedMint(creator, referral, sponsor, 1);
  ...
  function test_feeVerification(
    ...
    uint256 totalBps = uint80(uplinkBps) +
      uint80(channelBps) +
      uint80(creatorBps) +
