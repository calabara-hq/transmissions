...
interface IFees {
  ...
  function requestEthMint(
    ...
    address sponsor,
    uint256 amount
  ) external view returns (IRewards.Split memory);
...
contract CustomFees is IFees {
  ...
  function _requestMint(
    ...
    address sponsor,
    uint256 amount
  ) internal view returns (IRewards.Split memory) {
    ...
    if (feeConfig.mintReferralBps > 0) {
      if (referral == address(0)) {
        ...
        _allocations[commandCount] =
          amount *
          _calculateSplitFromBps(
            isNative ? feeConfig.ethMintPrice : feeConfig.erc20MintPrice,
            feeConfig.mintReferralBps
          );
        commandCount++;
      }
    ...
    if (feeConfig.channelBps > 0) {
      if (feeConfig.channelTreasury == address(0)) {
        ...
        _allocations[commandCount] =
          amount *
          _calculateSplitFromBps(isNative ? feeConfig.ethMintPrice : feeConfig.erc20MintPrice, feeConfig.channelBps);
        commandCount++;
      }
    }
