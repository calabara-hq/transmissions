...
contract DynamicLogic is ILogic, Ownable {
  ...
  function _validateLogic(
    ...
    require(
      targets.length == signatures.length &&
        ...
        literalOperands.length == interactionPowerTypes.length &&
   ...
   */
  function _loopAndExecuteLogic(InteractionLogic storage logic, address user) internal view returns (uint256[] memory) {
    ...
    for (uint256 i; i < length; i++) {
      bytes memory adjustedCalldata = _adjustCalldata(
        logic.datas[i],
        approvedSignatures[logic.signatures[i]].calldataAddressPosition,
        user
      );

      (bool success, bytes memory executionResult) = _executeLogic(
        logic.targets[i],
        logic.signatures[i],
        adjustedCalldata
      );
